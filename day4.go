...
var input = `[1518-05-27 00:42] falls asleep
...
type event struct {
	dt    datetime
	guard int
	t     int // 0=begin, 1=fall, 2=wake
}

func sleeps(evs []event) map[int][]int {
	lastGuard := -1
	sleepStart := -1
	rm := map[int][]int{}
	for _, ev := range evs {
		if ev.t == 0 {
			...
			if rm[lastGuard] == nil {
				rm[lastGuard] = make([]int, 60)
			}
			end := ev.dt.m
			if ev.dt.m == 0 {
				end = 60
			}
			for m := sleepStart; m < end; m++ {
				rm[lastGuard][m]++
			}
		}
	}
...
func main() {
	...
	for _, line := range lines {
		var dt datetime
		if _, err := fmt.Sscanf(line[:19], "[%d-%d-%d %d:%d]", &dt.Y, &dt.M, &dt.D, &dt.h, &dt.m); err != nil {
			panic(err)
		}
		right := line[19:]
		var ev = event{dt: dt}
		if right == "falls asleep" {
			ev.t = 1
		} else if right == "wakes up" {
			ev.t = 2
		} else if _, err := fmt.Sscanf(right, "Guard #%d", &ev.guard); err != nil {
			panic(err)
		}
		evs = append(evs, ev)
	}
